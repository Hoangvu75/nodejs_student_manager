...
export const removeStudent = async (req: any, res: any) => {
  try {
    Student.findByIdAndRemove(req.params.student_id).then((student: any) => {
      if (!student) {
...
export const updateStudent = async (req: any, res: any) => {
  try {
    ...
    await Student.findByIdAndUpdate(studentId, updatedStudentData, {
      new: true,
      runValidators: true,
    }).then((student: any) => {
      if (!student) {
...
export const findStudentById = async (req: any, res: any) => {
  try {
    const studentId = req.params.student_id;
    await Student.findById(studentId).then((student: any) => {
      if (!student) {
        return res.status(404).send({
          success: false,
